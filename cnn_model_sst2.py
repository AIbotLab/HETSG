...
class CNN(nn.Module):
	...
	def __init__(self, args, data, vectors):
		...
		self.fc = nn.Linear(len(self.kernel_sizes) * args.kernel_num, args.class_num)			
	...
	def forward(self, batch):
		...
		out = torch.cat(conv_result, 1)			# out: batch_size*(len(args.kernel_sizes)*args.kernel_num)
		out = F.dropout(out, p=self.args.dropout, training=self.training)
		out = self.fc(out)		# output: batch_size*class_num
